...
rosh my-game.rosh --target phaser --run
```

That creates a working space shooter — score, lives, player, hazards, bullets, explosions, sound — and opens it in the browser. Edit `my-game.rosh` to make it your own.

Full docs at [rosh.cloud/docs](https://rosh.cloud/docs) · [Syntax reference](https://rosh.cloud/docs/syntax)
...
| `add` | `add <item> to <list>` | `add visitor to visitors` |
...
# List operations
add "Alice" to visitors
remove "Alice" from visitors
...
These features require a [rosh.cloud](https://rosh.cloud) account. Local use (above) works without one.
...
Templates: `hello` (minimal), `game` (full space-shooter), `app` (interactive web app).
