...
class PhaserEmitter(BaseEmitter):
    ...
    def _emit_game_config(self):
        ...
        self.write("window.game = game;  // Expose globally for orientation handling")
        self.write_blank()
    ...
    def _emit_orientation_handling(self):
        ...
        # Safari fix: refresh Phaser scale manager after orientation settles
        self.write("function refreshGame() {")
        self.indent()
        self.write("// Tell Phaser to recalculate scale (fixes Safari)")
        self.write("if (window.game && window.game.scale) {")
        self.indent()
        self.write("window.game.scale.refresh();")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
        ...
        # Safari needs multiple checks as viewport values update slowly
        self.write("function checkOrientationWithRetry() {")
        self.indent()
        self.write("checkOrientation();")
        self.write("refreshGame();")
        self.write("// Safari: viewport values may still be stale, recheck")
        self.write("setTimeout(() => { checkOrientation(); refreshGame(); }, 100);")
        self.write("setTimeout(() => { checkOrientation(); refreshGame(); }, 300);")
        self.dedent()
        self.write("}")
        self.write_blank()

        # Initial check and event listeners
        self.write("// Check on load (with delay for Safari initial render)")
        self.write("checkOrientation();")
        self.write("setTimeout(checkOrientationWithRetry, 100);")
        self.write_blank()
        self.write("// Check on resize/orientation change")
        self.write("window.addEventListener('resize', checkOrientationWithRetry);")
        self.write("window.addEventListener('orientationchange', () => {")
        self.indent()
        self.write("// Safari fires this before viewport updates - use retry pattern")
        self.write("setTimeout(checkOrientationWithRetry, 50);")
        self.dedent()
        self.write("});")
    ...
    def _emit_console(self):
        ...
        self.write("  display: none; position: fixed; bottom: 0; left: 0; right: 0;")
        ...
        self.write("  border-top: 2px solid #0ff;")
        self.write("`;")
        ...
        self.write("// Initialize RoshNetwork for multiplayer (legacy inline adapter)")
        self.write("if (typeof RoshNetwork !== 'undefined') {")
        self.indent()
        self.write("RoshNetwork.init({")
        self.indent()
        ...
        self.write("adapter: {")
        self.indent()
        self.write_comment("createObject(type, id, data) - matches rosh-network.js expected signature")
        self.write("createObject: function(type, id, data) {")
        self.indent()
        self.write("data = data || {};")
        self.write("console.log('[Network] createObject:', type, id, data);")
        ...
        self.write("const gameWidth = scene.sys.game.config.width || 800;")
        self.write("const gameHeight = scene.sys.game.config.height || 600;")
        self.write("let x = gameWidth / 2, y = gameHeight / 2;")
        self.write("if (data.x !== undefined && data.z !== undefined) {")
        self.indent()
        self.write_comment("3D world coords - convert to 2D screen")
        self.write("x = ((data.x + 5) / 10) * gameWidth;")
        self.write("y = ((data.z + 5) / 10) * gameHeight;")
        self.dedent()
        self.write("} else if (data.x !== undefined) { x = data.x; }")
        self.write("if (data.y !== undefined && Math.abs(data.y) > 10) y = data.y;")
        self.write("const SIZE_MAP = { tiny: 0.25, small: 0.5, medium: 1, big: 2, large: 2, huge: 4 };")
        self.write("const scale = SIZE_MAP[data.size] || (typeof data.size === 'number' ? data.size : 1);")
        self.write("const size = scale * 50;")
        self.write("const colors = {red: 0xff0000, green: 0x00ff00, blue: 0x0000ff, yellow: 0xffff00, white: 0xffffff, orange: 0xff8800, purple: 0x8800ff, cyan: 0x00ffff, gray: 0x888888, grey: 0x888888};")
        self.write("const color = (typeof data.color === 'string') ? (colors[data.color] || 0x888888) : (data.color || 0x888888);")
        self.write("scene.roshObjects[id] = scene.add.rectangle(x, y, size, size, color);")
        self.write("scene.roshObjects[id].setData('_type', type);")
        self.write("scene.roshObjects[id].setData('_color', data.color || 'gray');")
        self.write("console.log('[Network] Created', id, 'at', x, y);")
        self.dedent()
        self.write("},")
        self.write("deleteObject: function(id) {")
        self.indent()
        self.write("const scene = game.scene.getScene('GameScene');")
        self.write("if (scene && scene.roshObjects && scene.roshObjects[id]) {")
        self.indent()
        self.write("scene.roshObjects[id].destroy();")
        self.write("delete scene.roshObjects[id];")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("},")
        self.write("moveObject: function(id, pos) {")
        self.indent()
        self.write("const scene = game.scene.getScene('GameScene');")
        self.write("const obj = scene && (scene.roshObjects && scene.roshObjects[id] || scene[id]);")
        self.write("if (obj) {")
        self.indent()
        self.write("if (pos.x !== undefined) obj.x = pos.x;")
        self.write("if (pos.y !== undefined) obj.y = pos.y;")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("},")
        self.write("setProperty: function(id, prop, value) {")
        self.indent()
        self.write("const scene = game.scene.getScene('GameScene');")
        self.write("const obj = scene && (scene.roshObjects && scene.roshObjects[id] || scene[id]);")
        self.write("if (!obj) return;")
        self.write("const colors = {red: 0xff0000, green: 0x00ff00, blue: 0x0000ff, yellow: 0xffff00, white: 0xffffff, orange: 0xff8800, purple: 0x8800ff, cyan: 0x00ffff, gray: 0x888888};")
        self.write("switch (prop.toLowerCase()) {")
        self.indent()
        self.write("case 'x': obj.x = parseFloat(value); break;")
        self.write("case 'y': obj.y = parseFloat(value); break;")
        self.write("case 'visible': obj.visible = value === 'true' || value === true; break;")
        self.write("case 'color': if (obj.setFillStyle) obj.setFillStyle(colors[value] || 0x888888); break;")
        self.write("case 'scale': obj.setScale(parseFloat(value)); break;")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("},")
        self.write("applyCapability: function(id, capability, value) {")
        self.indent()
        self.write("console.log('[Network] applyCapability:', id, capability, value, '(not implemented in legacy)');")
        self.dedent()
        self.write("},")
        self.write("toggleSpotlight: function(visible, target) {")
        self.indent()
        self.write("console.log('[Network] toggleSpotlight:', visible, target, '(no-op in 2D)');")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
    ...
    def _emit_fuzzy_find_helper(self):
        """Emit fuzzy object name resolution helper."""
        known_objects = [obj.name for obj in self.ir.objects if 'target' not in obj.properties]

        self.write("// Fuzzy object name resolution")
        self.write("function resolveObjectName(scene, inputName) {")
        self.indent()
        self.write(f"const knownObjects = {known_objects};")
        self.write("const runtimeObjects = scene.roshObjects ? Object.keys(scene.roshObjects) : [];")
        self.write("const allNames = [...knownObjects, ...runtimeObjects];")
        self.write_blank()
        self.write("// Exact match first")
        self.write("if (scene[inputName] || (scene.roshObjects && scene.roshObjects[inputName])) {")
        self.indent()
        self.write("return { resolved: true, name: inputName, obj: scene[inputName] || scene.roshObjects[inputName] };")
        self.dedent()
        self.write("}")
        self.write_blank()
        self.write("// Fuzzy substring matching")
        self.write("const lowerInput = inputName.toLowerCase();")
        self.write("const matches = allNames.filter(n => n.toLowerCase().includes(lowerInput) || lowerInput.includes(n.toLowerCase()));")
        self.write_blank()
        self.write("if (matches.length === 1) {")
        self.indent()
        self.write("const name = matches[0];")
        self.write("roshLog('[resolved: \"' + inputName + '\" -> \"' + name + '\"]', 'dim');")
        self.write("return { resolved: true, name: name, obj: scene[name] || scene.roshObjects[name] };")
        self.dedent()
        self.write("} else if (matches.length > 1) {")
        self.indent()
        self.write("return { resolved: false, multiple: matches };")
        self.dedent()
        self.write("}")
        self.write_blank()
        self.write("return { resolved: false, notFound: true };")
        self.dedent()
        self.write("}")
        self.write_blank()
        self.write("function showMultipleMatches(inputName, matches) {")
        self.indent()
        self.write("roshLog('Multiple matches for \"' + inputName + '\":', 'info');")
        self.write("matches.slice(0, 8).forEach(m => roshLog('  ' + m, 'dim'));")
        self.write("if (matches.length > 8) roshLog('  ... and ' + (matches.length - 8) + ' more', 'dim');")
        self.write("roshLog('Which one did you mean?', 'dim');")
        self.dedent()
        self.write("}")
    ...
    def _emit_console_commands(self):
        ...
        """
        # First emit the fuzzy matching helper
        self._emit_fuzzy_find_helper()
        self.write_blank()
        ...
        # get command - select an object
        self.write("else if (command === 'get' || command === 'select') {")
        self.indent()
        self.write("if (parts.length < 2) { roshLog('Usage: get <object>', 'error'); return; }")
        self.write("const inputName = parts[1];")
        self.write("const resolved = resolveObjectName(scene, inputName);")
        self.write("if (!resolved.resolved) {")
        self.indent()
        self.write("if (resolved.multiple) { showMultipleMatches(inputName, resolved.multiple); }")
        self.write("else { roshLog('Object not found: ' + inputName, 'error'); }")
        self.write("return;")
        self.dedent()
        self.write("}")
        self.write("roshLog('Selected: ' + resolved.name, 'info');")
        self.dedent()
        self.write("}")
        ...
        self.dedent()
        self.write("}")
        self.write("const name = resolved.name;")
        self.write("const obj = resolved.obj;")
        self.write("roshLog('=== ' + name + ' ===', 'info');")
        self.write_comment("Show description first if available")
        self.write("if (obj.description) {")
        self.indent()
        self.write("roshLog(obj.description);")
        self.dedent()
        self.write("}")
        self.write("roshLog('  position: (' + Math.round(obj.x) + ', ' + Math.round(obj.y) + ')', 'dim');")
        self.write("if (obj.visible !== undefined) roshLog('  visible: ' + obj.visible, 'dim');")
        self.write("if (obj.text !== undefined) roshLog('  text: \"' + obj.text + '\"', 'dim');")
        self.write_comment("Show credit if available")
        self.write("if (obj.credit) {")
        self.indent()
        self.write("roshLog('  credit: ' + obj.credit, 'dim');")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
        ...
        self.write_blank()
        self.write_comment("Resolve with fuzzy matching")
        self.write("const resolved = resolveObjectName(scene, name);")
        self.write("if (!resolved.resolved) {")
        self.indent()
        self.write("if (resolved.multiple) { showMultipleMatches(name, resolved.multiple); }")
        self.write("else { roshLog('Object not found: ' + name, 'error'); }")
        self.write("return;")
        self.dedent()
        self.write("}")
        self.write("name = resolved.name;")
        self.write("const obj = resolved.obj;")
        self.write_blank()
        ...
        self.write_comment("Lookup tables for local rendering (numeric values)")
        self.write("const colors = {red: 0xff0000, green: 0x00ff00, blue: 0x0000ff, yellow: 0xffff00, cyan: 0x00ffff, magenta: 0xff00ff, white: 0xffffff, black: 0x000000, orange: 0xff8800, purple: 0x8800ff, pink: 0xff88ff, grey: 0x888888, gray: 0x888888, gold: 0xffd700, silver: 0xc0c0c0, brown: 0x8b4513};")
        ...
        self.write_blank()
        self.write_comment("Track semantic names (strings) for network broadcast")
        self.write("let count = 1, colorName = 'green', sizeName = 'medium', shape = 'rectangle';")
        ...
        self.write_blank()
        self.write_comment("Parse adjectives - keep semantic string names")
        self.write("for (let i = 0; i < args.length - 1; i++) {")
        self.indent()
        self.write("const word = args[i].toLowerCase();")
        self.write("if (colors[word]) colorName = word;")
        self.write("else if (sizes[word]) sizeName = word;")
        self.dedent()
        self.write("}")
        self.write_blank()
        self.write_comment("Convert semantic names to numeric values for local rendering")
        self.write("const color = colors[colorName];")
        self.write("const size = sizes[sizeName];")
        self.write_blank()
        self.write("for (let i = 0; i < count; i++) {")
        self.indent()
        ...
        self.write_comment("Broadcast semantic values to network (not numeric)")
        self.write("if (typeof RoshNetwork !== 'undefined' && RoshNetwork.isConnected()) {")
        self.indent()
        self.write_comment("Send semantic strings: color='red', size='big' (not 0xff0000, 80)")
        self.write("RoshNetwork.broadcastCreate(name, {type: baseName, x: x, y: y, color: colorName, size: sizeName});")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
        ...
        self.write("roshLog('Object not found in roshObjects: ' + name, 'error'); return;")
        self.dedent()
        self.write("}")
