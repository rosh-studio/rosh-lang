...
class ThreeJSEmitter(BaseEmitter):
    ...
    def _emit_repl_console(self):
        ...
        # Project Twin - WebSocket state for shared worlds
        self.write("let twinSocket = null, twinUserId = null, twinWorldId = null;")
        self.write("const TWIN_SERVER = 'wss://rosh.cloud/ws/world/';")
        self.write_blank()
        ...
        self.write("let color = colors[data.color] || (typeof data.color === 'string' && data.color.startsWith('#') ? parseInt(data.color.slice(1), 16) : 0x00ff00);")
        self.write("const size = data.size || 1;")
        self.write("let geom, mesh;")
        self.write("const shapeType = data.type || 'cube';")
        self.write("if (shapeType === 'sphere' || shapeType === 'ball') geom = new THREE.SphereGeometry(size);")
        ...
        self.write("if (announce) log('🌐 Remote: ' + (data.created_by ? data.created_by.slice(0,4) : 'user') + ' created ' + id, 'cyan');")
        self.dedent()
        self.write("}")
        self.write_blank()

        # Project Twin - Broadcast object creation to shared world
        self.write("function twinBroadcastCreate(name, type, x, y, z, color, size) {")
        self.indent()
        self.write("if (twinSocket && twinSocket.readyState === WebSocket.OPEN) {")
        self.indent()
        self.write("const colorStr = typeof color === 'number' ? '#' + color.toString(16).padStart(6, '0') : color;")
        self.write("const msg = { type: 'CREATE', id: name, object_type: type, x, y, z, color: colorStr, size };")
        self.write("twinSocket.send(JSON.stringify(msg));")
        ...
        # Project Twin - Broadcast object deletion to shared world
        self.write("function twinBroadcastDelete(name) {")
        self.indent()
        self.write("if (twinSocket && twinSocket.readyState === WebSocket.OPEN) {")
        self.indent()
        self.write("twinSocket.send(JSON.stringify({ type: 'DELETE', id: name }));")
    ...
    def _emit_exec_command(self):
        ...
        # Project Twin - Connect to shared world
        self.write("else if (parts[0] === 'connect' || parts[0] === 'twin') {")
        self.indent()
        self.write("const worldId = parts[1] || 'default';")
        self.write("if (twinSocket && twinSocket.readyState === WebSocket.OPEN) {")
        self.indent()
        self.write("log('Already connected to world: ' + twinWorldId, 'warn');")
        self.write("log('Use \"disconnect\" first to leave current world', 'dim');")
        self.dedent()
        self.write("} else {")
        self.indent()
        self.write("log('Connecting to shared world: ' + worldId + '...', 'cyan');")
        self.write("try {")
        self.indent()
        ...
        self.write("if (msg.type === 'CONNECTED') {")
        self.indent()
        self.write("twinUserId = msg.user_id;")
        self.write("log('Connected to \"' + worldId + '\" as user ' + msg.user_id, 'ok');")
        self.write("log('Users online: ' + msg.user_count, 'dim');")
        self.write("// Restore existing objects from server state")
        self.write("if (msg.state && msg.state.objects) {")
        self.indent()
        self.write("const objCount = Object.keys(msg.state.objects).length;")
        self.write("if (objCount > 0) {")
        self.indent()
        self.write("log('Loading ' + objCount + ' shared object(s)...', 'dim');")
        self.write("for (const [id, data] of Object.entries(msg.state.objects)) {")
        self.indent()
        self.write("twinCreateObject(id, data, false);")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
        ...
        self.write("}")
        self.dedent()
        self.write("} else if (msg.type === 'USER_JOINED') {")
        self.indent()
        self.write("log('User ' + msg.user_id + ' joined (total: ' + msg.user_count + ')', 'cyan');")
        self.dedent()
        self.write("} else if (msg.type === 'USER_LEFT') {")
        self.indent()
        self.write("log('User ' + msg.user_id + ' left (total: ' + msg.user_count + ')', 'dim');")
        self.dedent()
        self.write("} else if (msg.type === 'OBJECT_CREATED' && msg.by !== twinUserId) {")
        self.indent()
        self.write("twinCreateObject(msg.id, msg.data, true);")
        self.dedent()
        self.write("} else if (msg.type === 'OBJECT_MOVED' && msg.by !== twinUserId) {")
        self.indent()
        self.write("const obj = scene.getObjectByName(msg.id);")
        self.write("if (obj) { obj.position.set(msg.x, msg.y, msg.z); }")
        self.dedent()
        self.write("} else if (msg.type === 'OBJECT_DELETED' && msg.by !== twinUserId) {")
        self.indent()
        self.write("const obj = scene.getObjectByName(msg.id);")
        self.write("if (obj) { scene.remove(obj); delete gameObjects[msg.id]; log('🌐 Remote: ' + msg.by.slice(0,4) + ' deleted ' + msg.id, 'warn'); }")
        self.dedent()
        self.write("} else if (msg.type === 'CHAT') {")
        self.indent()
        self.write("log('[' + msg.user_id + '] ' + msg.message, 'cyan');")
        self.dedent()
        self.write("} else if (msg.type === 'ERROR') {")
        self.indent()
        self.write("log('Error: ' + msg.message, 'err');")
        self.dedent()
        self.write("} else if (msg.type === 'WORLD_RESET') {")
        self.indent()
        self.write("// Remove all twin-created objects from scene")
        self.write("Object.keys(gameObjects).forEach(name => {")
        self.indent()
        self.write("const obj = gameObjects[name];")
        self.write("if (obj && obj.userData && obj.userData._twin) {")
        self.indent()
        self.write("scene.remove(obj);")
        self.write("delete gameObjects[name];")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("});")
        self.write("log('⚠️ World reset by ' + msg.by + ' (' + msg.deleted_count + ' objects cleared)', 'warn');")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("};")
        self.dedent()
        self.write("} catch (e) { log('Failed to connect: ' + e.message, 'err'); }")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")

        # Project Twin - Disconnect from shared world
        self.write("else if (parts[0] === 'disconnect') {")
        self.indent()
        self.write("if (twinSocket) {")
        self.indent()
        self.write("twinSocket.close();")
        self.write("log('Disconnected from shared world: ' + twinWorldId, 'ok');")
        self.write("twinSocket = null; twinUserId = null; twinWorldId = null;")
        self.dedent()
        self.write("} else {")
        self.indent()
        self.write("log('Not connected to any shared world', 'dim');")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")

        # Project Twin - Sync command (broadcast object creation)
        ...
        self.write("else {")
        self.indent()
        self.write("const data = { object_type: obj.userData._type || 'cube', x: obj.position.x, y: obj.position.y, z: obj.position.z, color: obj.material?.color?.getHexString() || 'ffffff', size: obj.scale?.x || 1 };")
        self.write("twinSocket.send(JSON.stringify({ type: 'CREATE', id: objName, ...data }));")
        ...
        # Project Twin - Reset world command (distinct from 'reset scene' which reloads)
        self.write("else if (parts[0] === 'reset' && parts[1] === 'world') {")
        self.indent()
        self.write("if (!twinSocket || twinSocket.readyState !== WebSocket.OPEN) {")
        self.indent()
        self.write("log('Not connected. Use \"connect\" first.', 'err');")
        ...
        self.write("} else {")
        self.indent()
        self.write("twinSocket.send(JSON.stringify({ type: 'RESET' }));")
        self.write("log('Reset command sent to world: ' + twinWorldId, 'ok');")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
