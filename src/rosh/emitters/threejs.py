...
class ThreeJSEmitter(BaseEmitter):
    ...
    def _emit_capability_runtime(self):
        ...
        self.write("return { ok: true, description: `${name}.x`, undo: () => { obj.position.x = prev; } };")
        self.dedent()
        self.write("}")
        ...
        self.write("return { ok: true, description: `${name}.y`, undo: () => { obj.position.y = prev; } };")
        self.dedent()
        self.write("}")
        ...
        self.write("return { ok: true, description: `${name}.z`, undo: () => { obj.position.z = prev; } };")
        self.dedent()
        self.write("}")
        self.write("if (prop === 'visible') {")
        self.indent()
        self.write("const prev = obj.visible;")
        self.write("obj.visible = value === true || value === 'true';")
        self.write("return { ok: true, description: `${name}.visible`, undo: () => { obj.visible = prev; } };")
        self.dedent()
        self.write("}")
        ...
        self.indent()
        self.write("return { ok: true, undo: result.undo, description: result.description || `${obj.name || '(object)'}.${prop}` };")
        self.dedent()
        self.write("}")
        ...
        self.write("target._color = val;")
        self.write("redrawTextSprite(target);")
        self.write("return { description, undo: () => { if (prev === undefined) delete target._color; else target._color = prev; redrawTextSprite(target); } };")
        self.dedent()
        ...
        self.write("return { description, undo: () => { if (target.material && target.material.color) target.material.color.setHex(prev); } };")
        self.dedent()
        ...
        self.write("target.userData.font_size = n;")
        self.write("redrawTextSprite(target);")
        self.write("return { description, undo: () => { if (prev === undefined) delete target.userData.font_size; else target.userData.font_size = prev; redrawTextSprite(target); } };")
        self.dedent()
        self.write("};")
        ...
        self.write("target._font = ctx.raw.trim();")
        self.write("redrawTextSprite(target);")
        self.write("return { description, undo: () => { if (prev === undefined) delete target._font; else target._font = prev; redrawTextSprite(target); } };")
        self.dedent()
        self.write("};")
        ...
        self.write("target._text = ctx.raw;")
        self.write("redrawTextSprite(target, ctx.raw);")
        self.write("return { description, undo: () => { target._text = prev; redrawTextSprite(target, prev); } };")
        self.dedent()
        self.write("};")
        ...
        self.write("if (nums.length === 1) { target.scale.set(nums[0], nums[0], nums[0]); }")
        self.write("else { target.scale.set(nums[0], nums[1] ?? nums[0], nums[2] ?? nums[0]); }")
        self.write("return { description, undo: () => { if (target.scale) target.scale.set(prev.x, prev.y, prev.z); } };")
        self.dedent()
        self.write("};")
        ...
        self.write("return { description, undo: restorePrev };")
        self.dedent()
        self.write("}")
        self.write("const nums = ctx.numbers;")
        ...
        self.dedent()
        self.write("}")
        self.write("capabilityState.spin.set(target, { x: speeds[0], y: speeds[1], z: speeds[2] });")
        self.write("target.userData._spin = speeds;")
        self.write("return { description, undo: restorePrev };")
        self.dedent()
        self.write("};")
        ...
        self.dedent()
        self.write("}")
        self.write("capabilityState.bounce.set(target, { amplitude, frequency: freq * Math.PI * 2, base: target.position.y, elapsed: 0 });")
        self.write("target.userData._bounce = { amplitude, freq };")
        self.write("return { description, undo: restorePrev };")
        self.dedent()
        self.write("};")
        ...
        self.indent()
        self.write("const hadPrev = !!prevSnapshot || !!prevUserData;")
        self.write("if (prevSnapshot && prevSnapshot.base && target.scale) target.scale.set(prevSnapshot.base.x, prevSnapshot.base.y, prevSnapshot.base.z);")
        self.write("capabilityState.pulse.delete(target);")
        self.write("delete target.userData._pulse;")
        self.write("if (!hadPrev) return { description };")
        self.write("return { description, undo: restorePrev };")
    ...
    def _emit_repl_console(self):
        ...
        self.write("function pushUndo(description, fn) {")
        self.indent()
        self.write("if (typeof fn !== 'function') return;")
        self.write("undoStack.push({ description: description || 'change', apply: fn });")
        ...
        self.write("const steps = Math.min(Math.max(1, count), undoStack.length);")
        self.write("for (let i = 0; i < steps; i++) {")
        self.indent()
        self.write("const entry = undoStack.pop();")
        self.write("if (!entry) break;")
        self.write("try {")
        self.indent()
        self.write("entry.apply();")
        self.write("log('Undo: ' + entry.description, 'ok');")
        self.dedent()
        self.write("} catch (err) {")
        self.indent()
    ...
    def _emit_fuzzy_matching(self):
        ...
        self.write("const KNOWN_COMMANDS = ['set', 'get', 'list', 'create', 'delete', 'clone', 'look', 'examine', 'inspect', 'help', 'prompt', 'save', 'load', 'capabilities', 'camera', 'undo'];")
        self.write_blank()
    ...
    def _emit_exec_command(self):
        ...
        self.indent()
        self.write("log('Commands: list, get, set, look/examine, create, delete, clone, prompt, save, load, undo, undo stack, camera reset, capabilities', 'cyan');")
        self.dedent()
        self.write("}")
        ...
        self.write("pushUndo('create ' + mesh.name, () => { scene.remove(mesh); });")
        ...
        self.write("pushUndo(\"delete '\" + parts[1] + \"'\", () => {")
        self.indent()
        self.write("(parent || scene).add(obj);")
        self.dedent()
        self.write("});")
        self.write("log(\"Deleted '\" + parts[1] + \"'\", 'ok');")
        self.dedent()
        self.write("}")
        ...
        self.write("pushUndo(\"clone '\" + parts[1] + \"'\", () => { scene.remove(clone); });")
        ...
        self.indent()
        self.write("pushUndo(coreResult.description, coreResult.undo);")
        ...
        self.indent()
        self.write("pushUndo(capResult.description, capResult.undo);")
        self.write("log('OK', 'ok');")
        self.write("return;")
        self.dedent()
        self.write("}")
