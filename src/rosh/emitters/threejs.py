...
class ThreeJSEmitter(BaseEmitter):
    ...
    def _emit_repl_console(self):
        ...
        self.write("if (announce) log('Shared: ' + id + ' created by ' + (data.created_by || 'remote'), 'cyan');")
        self.dedent()
        self.write("}")
        self.write_blank()
        ...
        self.write("twinSocket.send(JSON.stringify({ type: 'CREATE', id: name, object_type: type, x, y, z, color: colorStr, size }));")
        self.dedent()
        self.write("}")
        self.dedent()
        self.write("}")
    ...
    def _emit_exec_command(self):
        ...
        self.indent()
        self.write("const obj = scene.getObjectByName(msg.id);")
        self.write("if (obj) { scene.remove(obj); log('Object ' + msg.id + ' deleted by ' + msg.by, 'dim'); }")
        self.dedent()
