...
- All adapter callbacks implemented (createObject, deleteObject, moveObject,
  setProperty, applyCapability, toggleSpotlight)
- SIZE_MAP now includes `medium: 1` for parity with Phaser
- Position conversion now detects actual screen size from renderer
- window.selectedObject exported

EXPECTED 2D/3D DIFFERENCES (not bugs):
- Spotlight: Full implementation (3D lighting)
- Gravity/Physics: Custom physics in adapter (3D feature)
- Click-to-move: Raycasting to ground plane (3D feature)
- Selection highlight: Uses emissive (3D) vs setTint (2D) - platform-specific
- Metadata: Uses userData (Three.js API) vs getData (Phaser API)
