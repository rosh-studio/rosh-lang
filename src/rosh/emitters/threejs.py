...
class ThreeJSEmitter(BaseEmitter):
    ...
    def _emit_fuzzy_matching(self):
        ...
        self.write("if ((parts[0] === 'set' || parts[0] === 'get' || parts[0] === 'look' || parts[0] === 'examine' || parts[0] === 'delete' || parts[0] === 'remove' || parts[0] === 'reset' || parts[0] === 'hide' || parts[0] === 'show' || parts[0] === 'clone') && parts.length > 1) {")
        self.indent()
    ...
    def _emit_known_objects(self):
        ...
        for name, props in objects.items():
            ...
            if opacity is not None:
                entry = f"{name}: {{ shape: '{shape}', color: {color:#x}, scaleX: {scale_x}, scaleY: {scale_y}, scaleZ: {scale_z}, opacity: {opacity} }}"
            else:
                entry = f"{name}: {{ shape: '{shape}', color: {color:#x}, scaleX: {scale_x}, scaleY: {scale_y}, scaleZ: {scale_z} }}"
    ...
    def _emit_exec_command(self):
        ...
        self.dedent()
        self.write("}")
        self.write("else if (parts[0] === 'help' && (parts[1] === 'create' || parts[1] === 'clone')) {")
        self.indent()
        ...
        self.write("const modWords = ['big', 'large', 'small', 'tiny', ...Object.keys(colors), ...articles];")
        # Find the name - first word that isn't a modifier, shape keyword, or article
        self.write("let name = words.find(w => !modWords.includes(w) && !shapeWords.includes(w)) || words[words.length-1] || 'object';")
        self.write("let color = 0x00ff00, size = 1, shape = 'box';")
        ...
        self.write("if (preset) { shape = preset.shape; color = preset.color; }")
        ...
        self.write("mesh.userData._type = name;")  # Track type for get all
        ...
        self.write("pushUndo(\"delete '\" + parts[1] + \"'\", () => { (parent || scene).add(obj); }, removeObj);")
        ...
        self.write("else { log(count + ' ' + typeName + (count > 1 ? ' objects:' : ' object:'), 'cyan'); matches.forEach(n => log('  ' + n)); }")
        self.dedent()
        self.write("}")
        ...
        self.indent()
        self.write("const objName = parts[1];")
        ...
        self.write("if (!obj) { log('Not found: ' + objName, 'err'); }")
        self.write("else {")
        self.indent()
        ...
        self.write("return;")  # Don't also clone
        self.dedent()
        self.write("}")
        ...
        self.write("log('Selected ' + matches.length + ' ' + typeName + '(s): ' + matches.map(o => o.name).join(', '), 'ok');")
