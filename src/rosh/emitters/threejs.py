...
class ThreeJSEmitter(BaseEmitter):
    ...
    def _emit_repl_console(self):
        ...
        self.write("undoStack.push({ description: description || 'change', undo: undoFn, redo: typeof redoFn === 'function' ? redoFn : null });")
    ...
    def _emit_fuzzy_matching(self):
        ...
        self.write("if ((parts[0] === 'set' || parts[0] === 'get' || parts[0] === 'look' || parts[0] === 'examine' || parts[0] === 'delete' || parts[0] === 'remove' || parts[0] === 'reset' || parts[0] === 'hide' || parts[0] === 'show' || parts[0] === 'clone') && parts.length > 1 && !skipFuzzy.includes(parts[1])) {")
        self.indent()
    ...
    def _emit_exec_command(self):
        ...
        self.write("const words = bulkCreateMatch[2].trim().split(/\\s+/);")
        self.write("if (words.length > 0 && count > 0) {")
        self.indent()
        # Last word is type, rest are modifiers
        self.write("let typeName = words[words.length - 1].toLowerCase().replace(/s$/, '');")  # Singularize
        ...
        # Confirmation for >= 10 (if meta.confirm is true)
        self.write("if (count >= 10 && meta.confirm) {")
        self.indent()
        ...
        self.write("execute: () => {")
        self.indent()
        self.write("for (let i = 0; i < count; i++) execCommand(createCmd);")
        ...
        self.write("let typeName = bulkSetMatch[2].toLowerCase().replace(/s$/, '');")  # Singularize
        ...
        self.write("else {")
        self.indent()
        self.write("for (const obj of targets) execCommand('set ' + obj.name + ' ' + prop + ' to ' + value);")
        ...
        self.write("let typeName = bulkGetMatch[2].toLowerCase().replace(/s$/, '');")  # Singularize
        self.write("const typeObjs = [];")
        self.write("scene.traverse(o => { if (o.name === typeName || o.name.startsWith(typeName + '-') || o.userData._type === typeName) typeObjs.push(o); });")
        self.write("const targets = typeObjs.slice(0, count);")
        self.write("if (targets.length === 0) { log('No ' + typeName + ' objects found', 'err'); }")
        ...
        self.dedent()
        self.write("}")
        self.write("else if (parts[0] === 'help' && (parts[1] === 'look' || parts[1] === 'examine' || parts[1] === 'inspect')) {")
        self.indent()
        self.write("log('look/examine/inspect - Inspect an object', 'cyan');")
        self.write("log('Usage:', 'dim');")
        ...
        self.write("let name = (words[words.length - 1] || 'object').replace(/s$/, '');")
        ...
        self.write("log('Created ' + clone.name + ' (cloned from ' + name + ')', 'ok');")
        self.dedent()
        self.write("} else {")
        self.indent()
        ...
        self.write("log('Created ' + name + ' (fallback)', 'ok');")
        self.dedent()
        self.write("});")
        self.dedent()
        self.write("} else {")
        ...
        self.write("if (!isNaN(count) && parts[2]) {")
        self.indent()
        self.write("const typeName = parts[2].replace(/s$/, '');")  # Singularize
        ...
        # Confirmation for >= 10 (if meta.confirm is true)
        self.write("if (actualCount >= 10 && meta.confirm) {")
        self.indent()
        self.write("pendingOp = {")
        self.indent()
        ...
        self.write("execute: () => {")
        self.indent()
        self.write("toDelete.forEach(o => scene.remove(o));")
        ...
        self.write("else if (parts[1] === 'all' && parts[2]) {")
        self.indent()
        self.write("const typeName = parts[2].replace(/s$/, '');")  # Singularize
        self.write("const matches = findByType(typeName);")
        self.write("if (matches.length === 0) { log('No ' + typeName + ' objects found', 'err'); }")
        ...
        self.write("execute: () => {")
        self.indent()
        self.write("matches.forEach(o => scene.remove(o));")
        ...
        self.write("if (!obj && objName.endsWith('s')) {")
        self.indent()
        self.write("const singular = objName.slice(0, -1);")
        self.write("obj = scene.getObjectByName(singular);")
        self.write("if (obj) objName = singular;")
        self.dedent()
        self.write("}")
        self.write("if (obj) {")
        self.indent()
        ...
        self.indent()
        self.write("let typeName = parts[1];")
        # Handle plurals
        self.write("if (typeName.endsWith('ies')) typeName = typeName.slice(0, -3) + 'y';")
        self.write("else if (typeName.endsWith('es') && (typeName.endsWith('xes') || typeName.endsWith('shes') || typeName.endsWith('ches'))) typeName = typeName.slice(0, -2);")
        self.write("else if (typeName.endsWith('s') && !typeName.endsWith('ss')) typeName = typeName.slice(0, -1);")
        ...
        self.indent()
        self.write("const typeName = parts[2].replace(/s$/, '');")  # Singularize
        self.write("const modifier = parts.slice(3).join(' ');")
        ...
        self.indent()
        self.write("if (['big', 'bigger', 'large', 'larger'].includes(modifier)) { obj.scale.multiplyScalar(1.5); count++; }")
        self.write("else if (['small', 'smaller', 'tiny'].includes(modifier)) { obj.scale.multiplyScalar(1/1.5); count++; }")
        self.write("else if (modifier === 'visible' || modifier === 'shown') { obj.visible = true; count++; }")
        self.write("else if (modifier === 'invisible' || modifier === 'hidden') { obj.visible = false; count++; }")
        self.write("else if (knownColors[modifier]) { applyCapabilityBridge(obj, 'color', [modifier]); count++; }")
        self.dedent()
        self.write("}")
        ...
        self.write("}")
        # Confirmation for >= 10 (if meta.confirm is true)
        self.write("if (targets.length >= 10 && meta.confirm) {")
        self.indent()
        ...
        self.dedent()
        self.write("}")
        self.write("else if ((parts[0] === 'look' || parts[0] === 'examine') && parts.length < 2) {")
        self.indent()
        self.write("log('look/examine - Inspect objects', 'cyan');")
        self.write("log('Usage:', 'dim');")
